...
public class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle b){super.onCreate(b);
        ...
        TextView title=new TextView(this);title.setText("Stock Radar v0.3 · 마샬 패턴 스캐너");title.setTextSize(25);title.setTextColor(Color.rgb(14,53,93));root.addView(title);
        ...
        Button auto=new Button(this);auto.setText("⚡ 마샬 패턴 자동추천 TOP 5");auto.setOnClickListener(v->runMarshallTop());root.addView(auto);
        ...
        status=new TextView(this);status.setPadding(0,pad(10),0,pad(10));status.setText("자동추천은 거래량·기관·외국인·거래대금 후보를 모아서 정밀점수화합니다.");root.addView(status);
    ...
    private void runMarshallTop(){
        ...
        worker.submit(()->{try{
            ScannerRepository.ScanBundle b=repo().scanMarshallTop(5);
    ...
    private void addCard(ScanResult r){LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(pad(14),pad(12),pad(14),pad(12));GradientDrawableCompat.set(card,gradeColor(r.grade()));
        ...
        for(String x:r.positives()){TextView t=new TextView(this);t.setText("✓ "+x);t.setTextColor(Color.WHITE);card.addView(t);}for(String x:r.warnings()){TextView t=new TextView(this);t.setText("⚠ "+x);t.setTextColor(Color.rgb(255,230,160));card.addView(t);}LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,ViewGroup.LayoutParams.WRAP_CONTENT);lp.setMargins(0,0,0,pad(10));results.addView(card,lp);}
