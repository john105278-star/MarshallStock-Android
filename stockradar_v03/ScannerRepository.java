...
public final class ScannerRepository {
    ...
    public ScanBundle scanMarshallTop(int topN) throws Exception {
        ...
        addSeeds(seeds,client.fetchInstitutionBuyCandidates(12),4,"기관순매수상위");
        Thread.sleep(250);
        addSeeds(seeds,client.fetchForeignBuyCandidates(10),2,"외국인순매수상위");
        Thread.sleep(250);
        addSeeds(seeds,client.fetchVolumeSurgeCandidates(12),3,"거래량증가");
        Thread.sleep(250);
        addSeeds(seeds,client.fetchTradingValueCandidates(10),1,"거래대금상위");

        List<Seed> ordered=new ArrayList<>(seeds.values());
        ordered.sort(Comparator.comparingInt(Seed::seedScore).reversed());
        int deep=Math.min(12,ordered.size());
